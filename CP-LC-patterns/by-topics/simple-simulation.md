# Simple Simulation

## Collection of Similar Problems (can be done sequentially)

### LC 859 - Buddy Strings
**Link:** https://leetcode.com/problems/buddy-strings/  
**Platform:** LeetCode  
**Difficulty:** Easy  
**Tags:** String, Simulation, Case Analysis, Hash Table, Brute Force

### USACO 891 - Shell Game
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=891  
**Platform:** USACO  
**Difficulty:** Bronze (Jan 2019)  
**Tags:** Simulation, Brute Force, Implementation

### USACO 855 - Mixing Milk
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=855  
**Platform:** USACO  
**Difficulty:** Bronze (Dec 2018)  
**Tags:** Simulation, Implementation, Array

### LC 832 - Flipping an Image
**Link:** https://leetcode.com/problems/flipping-an-image/  
**Platform:** LeetCode  
**Difficulty:** Easy  
**Tags:** Array, Matrix, Simulation, Bit Manipulation

### USACO 665 - The Cow-Signal
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=665  
**Platform:** USACO  
**Difficulty:** Bronze (Dec 2016)  
**Tags:** Array, Matrix, Implementation, Simulation

### USACO 568 - Speeding Ticket
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=568  
**Platform:** USACO  
**Difficulty:** Bronze (Dec 2015)  
**Tags:** Array, Implementation, Simulation

### USACO 735 - The Lost Cow
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=735  
**Platform:** USACO  
**Difficulty:** Bronze (US Open 2017)  
**Tags:** Simulation, Math, Implementation, Greedy

### LC 1920 - Build Array from Permutation
**Link:** https://leetcode.com/problems/build-array-from-permutation/  
**Platform:** LeetCode  
**Difficulty:** Easy  
**Tags:** Array, Simulation, Permutation

### USACO 760 - The Bovine Shuffle
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=760  
**Platform:** USACO  
**Difficulty:** Bronze (Dec 2017)  
**Tags:** Array, Simulation, Permutation

### LC 253 - Meeting Rooms II
**Link:** https://leetcode.com/problems/meeting-rooms-ii/  
**Platform:** LeetCode  
**Difficulty:** Medium  
**Tags:** Interval, Sweep Line, Heap, Greedy, Sorting

### USACO 856 - The Bucket List
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=856  
**Platform:** USACO  
**Difficulty:** Bronze (Dec 2018)  
**Tags:** Simulation, Interval, Sweep Line, Array, Sorting

### USACO 917 - Measuring Traffic
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=917  
**Platform:** USACO  
**Difficulty:** Bronze (Feb 2019)  
**Tags:** Interval, Simulation, Constraint Propagation, Simulation, Greedy

### LC 396 - Rotate Function
**Link:** https://leetcode.com/problems/rotate-function/  
**Platform:** LeetCode  
**Difficulty:** Medium  
**Tags:** Array, Math

### USACO 616 - Circular Barn
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=616  
**Platform:** USACO  
**Difficulty:** Bronze (Feb 2016)  
**Tags:** Array, Math, Brute Force, Simulation

### USACO 664 - Block Game
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=664  
**Platform:** USACO  
**Difficulty:** Bronze (Dec 2016)  
**Tags:** String, Implementation, Simulation, Frequency Counting

### LC 1275 - Find Winner on a Tic Tac Toe Game
**Link:** https://leetcode.com/problems/find-winner-on-a-tic-tac-toe-game/   
**Platform:** LeetCode   
**Difficulty:** Easy   
**Tags:** Array, Matrix, Simulation, Hash Table

### USACO 831 - Team Tic Tac Toe
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=831  
**Platform:** USACO  
**Difficulty:** Bronze (US Open 2018)  
**Tags:** Array, Matrix, Simulation, Set

### USACO 593 - Mowing the Field
**Link:** https://usaco.org/index.php?page=viewproblem2&cpid=593
**Platform:** USACO
**Difficulty:** Bronze (Jan 2016)
**Tags:** Simulation, Hash Map, Implementation